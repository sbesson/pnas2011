...
warning off all
%drawProbabilityPhasediagram;
dtheta =pi/64;
angle = 120;
savefile = 'triangular_limits.mat';
if exist(savefile,'file') ==2, load(savefile); end
    
if ~exist('eqline1','var')
    smin = fminsearch(@(s) Ldiff(s,[1 0 -1])^2 + Ldiff(s,[1 -1 0])^2,[pi/3 pi/3]);
    theta1c = smin(1);
    theta2c = smin(2);
    plot(theta1c,theta2c,'or');

    eqline1(:,2) = dtheta:dtheta:theta2c; 
    eqline1(:,1) = arrayfun(@(theta2) fzero(@(theta1) Ldiff([theta1 theta2],[-1 1 0]),theta2),eqline1(:,2) );
    eqline1(end+1,1) = theta1c;
    eqline1(end,2) = theta2c;

    eqline2(:,2) = pi-dtheta:-dtheta:theta2c; 
    eqline2(:,1) = arrayfun(@(theta2) fzero(@(theta1)  Ldiff([theta1 theta2],[-1 0 1]),pi/2-theta2/2),eqline2(:,2));
    eqline2(end+1,1) = theta1c;
    eqline2(end,2) = theta2c;
    eqline2 = eqline2(end:-1:1,:);


    eqline3(:,1)=[theta1c pi];
    eqline3(:,2)=[theta2c 0];
...
%     plot(x2,y2,'or');
    ...
    x1 = arrayfun(@(theta2) fzero(@(theta1) Ldiff([theta1 theta2],[1 -1 0])-l,theta2+pi/8),y1);

    x2 = pi/2:-dtheta:theta1c; 
